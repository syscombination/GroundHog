    ...
    def search(self, seq, systems, n_samples, empty_p=0.05,weight_m=1.,ignore_unk=False, minlen=1):
        ...
        for k in xrange(len(systems[0])):
            ...
            for i in range(probs.shape[0]):
            	probs[i][self.state['empty_sym_target']] = -numpy.inf
            #print trans
            #print states
            #print last_words
            #print last_refs
...
def main():
    ...
    if args.source and args.trans and args.system:
        ...
        for i, line in enumerate(fsrc):
            ...
            systems = []
            for snum in xrange(state['num_systems']):
                systems.append(parse_input(state, indx_word_t, fsystems[snum].readline(), idx2word=idict_tgt)[0])
            if args.verbose:
